...
build: vendor
	...
	GOOS=darwin go build -o bin/darwin/$(HOST_GOARCH)/kubelang -ldflags "-X main.version=$(VERSION)" ./cmd
	GOOS=linux go build -o bin/linux/$(HOST_GOARCH)/kubelang -ldflags "-X main.version=$(VERSION)" ./cmd
